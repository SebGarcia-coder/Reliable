...
import { QuestionType, RulesText } from '@/enums/game'
...
const handleButtonClick = () => {
  if (gameStore.currentQuestionNumber === 3) {
    gameStore.moveToStep(3)
  } else if (gameStore.currentQuestionNumber === 6) {
    gameStore.moveToStep(5)
  } else {
...
<template>
  <div v-if="gameStore.currentGameStep === 0">
    <Rules
      :text="RulesText.STEP_0"
      @nextStep="gameStore.moveToStep(1)"
    />
  </div>
  <div v-else-if="gameStore.currentGameStep === 1">
      <Rules
        :text="RulesText.STEP_1"
        @nextStep="gameStore.moveToStep(2)"
      />
    </div>
    <div v-else-if="gameStore.currentGameStep === 3">
      <Rules
        :text="RulesText.STEP_3"
        @nextStep="gameStore.moveToStep(4)"
      />
    </div>
    <div v-else-if="gameStore.currentGameStep === 5">
      <Rules
        :text="RulesText.STEP_5"
    ...
    <div class="flex flex-col items-center md:justify-center min-h-screen">
      <div class="flex flex-col space-y-2 max-w-[90%] md:max-w-3xl mb-6">
        <div v-if="!gameStore.demoMode" class="flex items-center md:justify-end justify-center">
          <Timer
            v-if="gameStore.answerSubmitted === false && !gameStore.demoMode"
        ...
        <div
          class="flex flex-wrap md:justify-between justify-between gap-2 md:gap-6 md:h-[150px] h-auto"
        >
